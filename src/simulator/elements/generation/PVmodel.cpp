...
/******************************************************************************/
/* RADIATION */
void CPVmodel::_GlobalRadiation( double* pf_Gobs,datePV s_date, geoCoord s_coord, orientation sf_orientation, double f_reflectivity, double f_dirt_factor ){
...
//	nDayYear = DayOfYear (s_date);

//	fGamma = Gamma(nDayYear);
//	fDelta = Delta(fGamma);
//	
//	
...
//		
//		m_pfGTH[i] = fBhab + fDhCSab + fDhISOab + fDhHORab + fALhab;
...
//	fSolH = (double)sn_date.hour + ((double) SOLMINUTE) / 60 - 12.0 + fTE - (double)TZDiffHour(n_day) - (sf_coord.timeZone - sf_coord.longitude) / 15;
//	
...
//	if( ComparisonDouble(f_latitude,(double)MADRIDLATITUDE,0.2) ){
//		if (f_kth < 0.2) 
//		{
...
//	a = 2 * (1 - cos((double) DEG_TO_RAD * HALFANGLE))* ChiC(f_thetaZS, f_thetaS);
//	c = 2 * (1 - cos((double) DEG_TO_RAD * HALFANGLE))* ChiH(f_thetaZS);
